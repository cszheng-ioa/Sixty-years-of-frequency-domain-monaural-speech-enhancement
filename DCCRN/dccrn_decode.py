...
def enhance(args):
    model = DCCRN(rnn_units=256, masking_mode='E', use_clstm=True, kernel_num=[32, 64, 128, 256, 256, 256]).cuda()
    model.load_state_dict(torch.load('./BEST_MODEL/wsj0_si84_300h_dccrn_cprs_model.pth'))
    print(model)
    model.eval()
    model.cuda()
...
#            feat_wav = torch.FloatTensor(feat_wav)
            ...
            # compress
            feat_x, phase_x = torch.norm(feat_x, dim=1) ** 0.5, torch.atan2(feat_x[:, 1, :, :], feat_x[:, 0, :, :])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--esti_clean_file_path', type=str,
                        default='./datasets/wsj0_si84_300h_dccrn_noncprs_model')
    ...
    parser.add_argument('--esti_clean_file_path', type=str,
                        default='./datasets/wsj0_si84_300h_dccrn_cprs_model')
    parser.add_argument('--fs', type=int, default=16000)
    args = parser.parse_args()
    enhance(args=args)
