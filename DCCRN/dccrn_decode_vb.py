...
def enhance(args):
    ...
    model.load_state_dict(torch.load('./BEST_MODEL/vb_dccrn_noncprs_model.pth'))
    print(model)
    model.eval()
    model.cuda()
...
#            feat_wav = torch.FloatTensor(feat_wav)
            ...
            # compress
            feat_x, phase_x = torch.norm(feat_x, dim=1) ** 1.0, torch.atan2(feat_x[:, 1, :, :], feat_x[:, 0, :, :])
...
#            esti_utt = esti_utt[:wav_len]
            ...
            esti_utt = librosa.istft((de_esti), hop_length=128,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--esti_clean_file_path', type=str,
                        default='./datasets/vb_dccrn_noncprs_model_try')
