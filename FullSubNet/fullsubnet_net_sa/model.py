...
from fullsubnet_net_sa.feature import drop_band
from fullsubnet_net_sa.base_model import BaseModel
from fullsubnet_net_sa.sequence_model import SequenceModel
...
class Model(BaseModel):
    ...
    def forward(self, noisy_mag):
        ...
        # 这种方式相当于输出第k帧对应的是输入的第K+look_ahead的帧，因此能用到一部分的未来信息
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        x = torch.rand(3, 1, 257, 64)
        y = model(x)
        print("{}->{}".format(x.shape, y.shape))
        macs, params = get_model_complexity_info(model, (1, 257, 64), as_strings=True,
