...
def si_sdr_loss(esti, label, EPSILON=2e-7):
    # cal SI-SDR
    a = torch.sum(esti * label, dim=-1, keepdim=True) / (torch.sum(label ** 2, dim=-1, keepdim=True) + EPSILON)
    xa = a * label
    xay = xa - esti
    d = torch.sum(xa ** 2, dim=-1, keepdim=True) / (torch.sum(xay ** 2, dim=-1, keepdim=True) + EPSILON)
    d = -torch.mean(10 * torch.log10(d))
    return d
