...
class DeepXi():
	...
	def __init__(
		...
		**kwargs
		...
		if os.path.exists(inp_tgt_obj_path) and not reset_inp_tgt:
			...
			print(s_sample.dtype)
			print(d_sample.dtype)
		...
		get_flops(self.model)


		# flops = tf.compat.v1.profiler.profile(self.network, options=tf.compat.v1.profiler.ProfileOptionBuilder.float_operation())
	...
	def train(
		...
		if val_flag:
			val_set = self.val_batch(val_save_path, val_s, val_x, val_s_len, val_x_len, val_snr)
		...
		if eval_example:
			...
			# 	self.train_x_list[0:mbatch_size])
			s, x, s_len, x_len, _ = self.wav_batch(self.train_s_list[0:mbatch_size],
				self.train_x_list[0:mbatch_size])
			(x, s, d) = self.inp_tgt.add_noise_batch(self.inp_tgt.normalise(s),
				self.inp_tgt.normalise(x), s_len, x_len, 0)
			# (x, s, d) = self.inp_tgt.add_noise_batch(s,
			# 	x, s_len, x_len, 0)
			for (i, _) in enumerate(s):
				snr_tgt = process_txt(os.path.basename(self.train_x_list[i]['file_path']),1, '_')
				snr_tgt = np.array(snr_tgt, dtype=np.float32)
				snr_act = self.inp_tgt.snr_db(s[i][0:s_len[i]], d[i][0:x_len[i]])
				print('SNR target|actual: {:.2f}|{:.2f} (dB).'.format(snr_tgt, snr_act))
	...
	def sample(
		...
		if os.path.exists(sample_path + '.npz'):
			...
			# x_sample_list = random.sample(self.train_x_list, sample_size)
			s_sample_list = self.train_s_list
			x_sample_list = self.train_x_list
			s_sample_int, x_sample_int, s_sample_len, x_sample_len, snr_sample = self.wav_batch(s_sample_list,
				x_sample_list)
#			print(s_sample_int.shape)
#			print(x_sample_len)
#			print(snr_sample)
...
#			x_sample = np.zeros_like(s_sample_int, np.float32)
			...
			# 	s, d, x, _ = self.inp_tgt.mix(s_sample_int[i:i+1], x_sample_int[i:i+1],
			# 		s_sample_len[i:i+1], x_sample_len[i:i+1], snr_sample[i:i+1])
			# 	s_sample[i, 0:s_sample_len[i]] = s
			# 	d_sample[i, 0:s_sample_len[i]] = d
			# 	x_sample[i, 0:s_sample_len[i]] = x
			s_sample = s_sample_int.astype(np.float32)
			d_sample = (x_sample_int-s_sample_int).astype(np.float32)
			x_sample = x_sample_int.astype(np.float32)
	...
	def mbatch_gen(self, n_epochs):
		...
		for _ in range(n_epochs):
			...
			# np.random.shuffle(self.train_s_list)
			# np.random.set_state(state)
			# np.random.shuffle(self.train_x_list)
			tmp = list(zip(self.train_s_list, self.train_x_list))
			random.shuffle(tmp)
			self.train_s_list[:], self.train_x_list = zip(*tmp)
			start_idx, end_idx = 0, self.mbatch_size
			for _ in range(self.n_iter):
				s_mbatch_list = self.train_s_list[start_idx:end_idx]
				# x_mbatch_list = random.sample(self.train_x_list, end_idx-start_idx)
				x_mbatch_list = self.train_x_list[start_idx:end_idx]
				# print('s_list', s_mbatch_list)
				# print('x_list', x_mbatch_list)
				s_mbatch, x_mbatch, s_mbatch_len, x_mbatch_len, snr_mbatch = \
					self.wav_batch(s_mbatch_list, x_mbatch_list)
				inp_mbatch, xi_bar_mbatch, n_frames_mbatch = \
					self.inp_tgt.example(s_mbatch, x_mbatch, s_mbatch_len,
					x_mbatch_len, snr_mbatch)
				# print(n_frames_mbatch)
	...
	def val_batch(
		...
		for i in tqdm(range(batch_size)):
			inp, tgt, _ = self.inp_tgt.example(val_s[i:i+1], val_x[i:i+1],
				val_s_len[i:i+1], val_x_len[i:i+1], val_snr[i:i+1])
			...
			inp_batch[i,:,:] = inp.numpy()
			if tf.is_tensor(tgt): tgt_batch[i,:,:] = tgt.numpy()
			else: tgt_batch[i,:,:] = tgt
	...
	def wav_batch(self, s_list, x_list):
		"""
		...
		x_batch = np.zeros([batch_size, max_len], np.int16)
		...
		for i in range(batch_size):
			...
			# flag = True
			# while flag:
			# 	if x_list[i]['wav_len'] < s_batch_len[i]: x_list[i] = random.choice(self.train_x_list)
			# 	else: flag = False
			# (wav, _) = read_wav(x_list[i]['file_path'])
			# rand_idx = np.random.randint(0, 1+x_list[i]['wav_len']-s_batch_len[i])
			# x_batch[i,:s_batch_len[i]] = wav[rand_idx:rand_idx+s_batch_len[i]]
			(wav, _) = read_wav(x_list[i]['file_path'])
			x_batch[i,:x_list[i]['wav_len']] = wav

		x_batch_len = s_batch_len
		...
		# return s_batch, x_batch, s_batch_len, x_batch_len, snr_batch
		return s_batch, x_batch, s_batch_len, x_batch_len, 0
