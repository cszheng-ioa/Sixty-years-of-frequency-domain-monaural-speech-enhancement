...
class InputTarget(AnalysisSynthesis):
	...
	def mix(self, s, x, s_len, x_len, snr):
		"""
		Mix the clean speech and noise at SNR level, and then perform STFT analysis.
		...
		Returns:
			...
			n_frames - number of time-domain frames.
		"""
		s, x = self.normalise(s), self.normalise(x)
		n_frames = self.n_frames(s_len)
		if tf.rank(s) == 2:
			x, s, d = self.add_noise_batch(s, x, s_len, x_len, snr)
		elif tf.rank(s) == 1:
			x, d = self.add_noise(s[:s_len], x[:x_len], s_len, x_len, snr)
			s = s[:s_len]
		else: raise ValueError("Waveforms are of incorrect rank.")
		return s, d, x, n_frames
	...
	def add_noise_batch(self, s, x, s_len, x_len, snr):
		"""
		...
		# 	tf.reduce_max(s_len)), (s, x, s_len, x_len, snr), dtype=(tf.float32, tf.float32,
		# 	tf.float32), back_prop=False)
		# return tf.nest.map_structure(tf.stop_gradient, tf.map_fn(lambda z: self.add_noise_pad(z[0], z[1], z[2], z[3], z[4],
		# 	tf.reduce_max(s_len)), (s, x, s_len, x_len, snr), dtype=(tf.float32, tf.float32,
		# 	tf.float32)))

		(x, s, d) = self.add_noise_pad(s, x, s_len, x_len, snr, tf.reduce_max(s_len))
		return (x, s, d)

	def add_noise_pad(self, s, x, s_len, x_len, snr, pad_len):
		"""
		...
		"""
		# s, x = s[...,:s_len[0]], x[...,:x_len[0]]
		# # (x, d) = self.add_noise(s, x, s_len, x_len, snr)
		# d = tf.subtract(x, s)
		# total_zeros = tf.subtract(pad_len, s_len)
		# print(total_zeros)
		# print('s_len', s_len)
		# x = tf.pad(x, [[0, 0], [0, total_zeros[0]]], "CONSTANT")
		# s = tf.pad(s, [[0, 0], [0, total_zeros[0]]], "CONSTANT")
		# d = tf.pad(d, [[0, 0], [0, total_zeros[0]]], "CONSTANT")
	...
	# def add_noise(self, s, x, s_len, x_len, snr):
	# 	"""
	# 	Adds noise to the clean speech at a specific SNR value. A random section
	# 	of the noise waveform is used.
	#
	...
	# 	# snr = tf.cast(snr, tf.float32)
	# 	# snr = tf.pow(self.ten, tf.truediv(snr, self.ten)) # inverse of dB.
	# 	i = tf.random.uniform([1], 0, tf.add(1, tf.subtract(x_len, s_len)), tf.int32)
	# 	x = tf.slice(x, [i[0]], [s_len])
	# 	# P_s = tf.reduce_mean(tf.math.square(s), 0) # average power of clean speech.
	# 	# P_x = tf.reduce_mean(tf.math.square(x), 0) # average power of noise.
	# 	# alpha = tf.math.sqrt(tf.truediv(P_s,
	# 	# 	tf.maximum(tf.multiply(P_d, snr), 1e-12))) # scaling factor.
	# 	# d =	tf.multiply(d, alpha)
	# 	# x = tf.add(s, d)
